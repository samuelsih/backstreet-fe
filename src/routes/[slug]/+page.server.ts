...
export const load = (async (event) => {
	...
	try {
		const fetched = await axios.get<FindSlugResponse>(
			`${SECRET_API_GET_DATA}/${event.params.slug}`
		);
		response = fetched.data;
	} catch (err: unknown) {
		if (axios.isAxiosError(err)) {
			const { code, message } = err.response?.data || `Can't find ${event.params.slug} right now.`;
